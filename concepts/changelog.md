...
| Addition | Beta | Introduced the new [activityStatistics resource type](/graph/api/resources/activitystatistics?view=graph-rest-beta) that represents time spent by a user on various work activities during and outside of working hours, for the specified date range in the request. |
