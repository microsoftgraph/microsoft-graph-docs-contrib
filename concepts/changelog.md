...
| Addition | beta | [tokenIssuancePolicy](/graph/api/resources/tokenissuancepolicy?view=graph-rest-beta) resource type controls the characteristics of SAML tokens issued by Azure AD. Allows to set the signing algorithm, signing options or SAML token version to be used to issue the SAML token.
