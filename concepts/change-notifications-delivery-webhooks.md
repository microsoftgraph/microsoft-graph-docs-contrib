...
Once the Microsoft Graph change notifications service receives a 2xx class code from your endpoint within 3 seconds, the notification is considered sent.  The service continues to try to deliver the notification for up to 4 hours for any other HTML response or if notification is timed out.  For notifications that are retried, the timeout is extended to 10 seconds.
...
1. An endpoint is marked "slow" once more than 10% of responses take longer than the 3s timeout allowance in a 10-minute window.
