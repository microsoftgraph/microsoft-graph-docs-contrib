...
Microsoft Graph exposes an endpoint to retrieve all pages but this is not the recommended way to get all the pages the user has access to. When the user has too many sections and an attempt is made to get all pages, you will see a response with HTTP status code `400` and the following message: "The maximum number of sections is exceeded for this request. To get pages for accounts with a high number of sections, we recommend getting pages for one section at a time." For more information about this error code, see [OneNote error codes](/graph/onenote-error-codes#20266).

In order to get all pages that a user has access to, the recommended approach is to make separate get pages calls per section.
