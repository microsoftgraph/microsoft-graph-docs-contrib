...
3. Add all members (and guests if necessary) to the group using the [add member](/graph/api/group-post-members?view=graph-rest-1.0) operation, if you did not do so in Step 1. If adding multiple members, please add a 1 second delay after each add operation. 
