...
We monitor the resource usage of multiple dependency services when the APIs are called, and the impact varies between different requests. For example, you may easily imagine that updating a short string in a single cell of a small workbook would consume much less resources than adding a big table with complex formulas to a large workbook. Even with the same API, parameters and target workbooks can introduce significant differences. Using simple and universal numbers would result in more restrictive limits; in contrast, the following practices will help Microsoft Graph consumers complete tasks quickly with fewer failures.
