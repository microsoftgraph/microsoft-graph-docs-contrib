...
Change notifications enables you to listen to changes (creation, update and deletion) to [messages](/graph/resources/chatMessage?view=graph-rest-1.0) in [channel](/graph/resources/channel?view=graph-rest-1.0) or [chat](/graph/resources/chat?view=graph-rest-1.0).

Change notifications enables you to listen to messages in a low latency model by maintaining a [subscription](/graph/resources/webhooks?view=graph-rest-1.0). You can additionally get the resource data in the notifications and thus avoiding the need to call into API to get the payload.
...
To track all changes related to messages at tenant level you can use subscriptions at tenant level for channel and chat messages. This requires creation of 2 subscriptions, one to track all messages across [channels](/graph/resources/channel?view=graph-rest-1.0) and other to track all messages across [chats](/graph/resources/chat?view=graph-rest-1.0).
