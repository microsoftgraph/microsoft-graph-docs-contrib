...
+ [Applications](/graph/api/resources/application)
+ [Contacts](/graph/api/resources/contact)
+ [Devices](/graph/api/resources/device)
+ [Groups](/graph/api/resources/group)
+ [Delegated permissions grants](api/resources/oauth2permissiongrant)
+ [Service Principals](/graph/api/resources/serviceprincipal)
...
+ ![Works by default. Does not require advanced query parameters.](../concepts/images/advanced-query-parameters/default.png) The `$filter` operator works by default for that property.
+ ![Requires advanced query parameters.](../concepts/images/advanced-query-parameters/advanced.png) The `$filter` operator **requires** *advanced query parameters*, which are:
  + `ConsistencyLevel=eventual` header
  + `$count=true` query string
+ ![Not supported.](../concepts/images/advanced-query-parameters/notSupported.png) The `$filter` operator is not supported on that property yet. Please [send us feedback](https://aka.ms/MsGraphAADSurveyDocs) if you need this property to enable your scenarios.
