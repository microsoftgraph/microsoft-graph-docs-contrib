...
+ The `NOT` and `ne` negation operators are supported only in advanced queries. In `$filter` expressions, the syntax for using the `ne` operator is similar to that of the `eq` operator, except for queries that use the `any` lambda operator which use the `NOT` operator instead. See [Filter using lambda operators](/graph/query-parameters#filter-using-lambda-operators).
