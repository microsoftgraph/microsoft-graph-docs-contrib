...
Often, apps execute business logic based on resource data included in notifications. First verifying the authenticity of each notification is important. Otherwise, a third party could spoof your app with false notifications, make it execute its business logic incorrectly, and lead to a security incident.

For basic notifications which do not contain resource data, simplify validation based on the **clientState** value as described [here](webhooks.md#processing-the-notification). This is acceptable, as you can make subsequent trusted Microsoft Graph calls to get access to resource data, and therefore the impact of any spoofing attempts is limited. 
