...
5. Take a closer look at the **context** field of the request that you are interested in.
   ...
   You get a response that looks like the following:
...
3. To revoke a previously approved request, select the approved request that needs to be revoked, and then choose **Revoke**. The next attempt to move data by using that approval will fail.

### Approval behavior

Microsoft Graph Data Connect approval requests have particular characteristics that are important to be aware of:
