...
- Basic datasets - Generated from raw customer-created content and inputs from Microsoft 365 applications and services (for example, Azure Active Directory, Outlook, Teams datasets). 
- Cleaned datasets - Generated by either normalization and de-duplication from basic datasets, or created from user activity or behavior signals in Microsoft 365 (for example, SharePoint, Office 365 datasets).  
- Curated datasets - Custom-generated for a specific use case or analytics scenarios, or generated from first-party Microsoft 365 analytics applications (for example, Viva Insights metrics). 
...
For information on datasets that are generally avaialble or in preview only, see [Dataset, regions, and sinks](/graph/data-connect-datasets#datasets).
