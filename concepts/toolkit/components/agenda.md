...
| preferred-timezone | preferredTimezone | Name of the IANA time zone to use when displaying events retrieved from Microsoft Graph; for example, `America/Los_Angeles`, see this [wikipedia article for a reference list of IANA time zones](https://en.wikipedia.org/wiki/List_of_tz_database_time_zones)  . By default, events render using the current time zone settings of the device being used. |
