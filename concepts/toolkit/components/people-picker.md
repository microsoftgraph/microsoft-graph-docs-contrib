...
| user-filters               | userFilters             | Specifies the filter criteria to use when querying the users' endpoint. It requires the `user-type` to be set to `user` or `contact`. By default, the `user-type` is `any` and this leads the querying to take place in the `people` endpoint block. For example, `user-filters="startsWith(displayName,'a')"`. This attribute is optional. Learn more about [the support for filter on user properties of directory objects](/graph/aad-advanced-queries?tabs=http#user-properties). <br /> <br /> When using only the `User.ReadBasic.All` permission scope, the list of available properties is limited and the component will adapt accordingly. The available properties will respect the [documented ones](/graph/permissions-reference#userreadbasicall) and can only query the following: `id`, `displayName`, `givenName`, `mail`, `securityIdentifier`, `surname` and `userPrincipalName`. As the component by default leverages the `jobTitle`, in cases where `User.ReadBasic.All` is being used, the `mail` property will be used as fallback. All the three and four line variants of the person template won't have data to show. To query other properties, using the `User.Read.All` permission scope is required.                                                                                                                                                                                                                                                                                        |
