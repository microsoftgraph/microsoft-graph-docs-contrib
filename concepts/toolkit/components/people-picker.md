...
| type                       | type                    | The type of entities to search for. Available options are: `person`, `group`, `any`. Default value is `person`. If this is set to `group` and either `group-id` or `group-ids` are set, then `userFilters` and `peopleFilters` have no effect.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
This component may make many queries depending on configuration and state. The Microsoft Graph APIs and permissions required have been split into three sections here for easier understanding. For each API called the user must have at least one of the listed permissions.
...
| `default-selected-group-ids` set                                                                       | GroupMember.Read.All, Group.Read.All, Directory.Read.All, Group.ReadWrite.All, Directory.ReadWrite.All    | [/groups](/graph/api/group-list)         | When `people-filters` is set this is added as the `$filter` parameter to the request |
| When a configuration below depends on `user-ids` being set, if there is an entry of `me` in `user-ids` | User.Read, User.ReadWrite                                                                                 | [/me](/graph/api/user-get)               |  |
...
| `type` set to `person` or `any` and either `user-filters` is not set or `user-type` is set to neither `user` nor `contact`                | People.Read, People.Read.All                                                                              | [/me/people](/graph/api/user-list-people) | When `people-filters` is set or `user-type` is not `any` a $filter parameter is added to the request. If `user-type` is not `contact` the `X-PeopleQuery-QuerySources: 'Mailbox,Directory'` header is set on the request |

### When a user has supplied a search term

| Configuration                                                                                                                                 | Permission                                                                                                | API                                                                       |  Additional options |
| -------------------------------------------------------------------------------                                                               | ------------------------------------------------                                                          | ------------------------------------------------------------------------- |---------------------|
| `group-id` is set                                                                                                                             | GroupMember.Read.All, Group.Read.All, Directory.Read.All, GroupMember.ReadWrite.All, Group.ReadWrite.All  | [/groups/${groupId}/members](/graph/api/group-list-members) | When `type` is `person` or `group` either `/microsoft.graph.user` or `/microsoft.graph.group` will be appended to the request path, a `$filter` parameter is composed with the user input value |
| `group-id` is set and `transitive-search` is true                                                                                             | GroupMember.Read.All, Group.Read.All, Directory.Read.All, GroupMember.ReadWrite.All, Group.ReadWrite.All  | [/groups/${groupId}/transitiveMembers](/graph/api/group-list-transitivemembers)  | When `type` is `person` or `group` either `/microsoft.graph.user` or `/microsoft.graph.group` will be appended to the request path, a `$filter` parameter is composed with the user input value |
| `group-id` is not set and `type` set to `person` or `any` and `user-type` set to `any` and `group-ids` is set                                 | GroupMember.Read.All, Group.Read.All, Directory.Read.All, GroupMember.ReadWrite.All, Group.ReadWrite.All  | [/groups/${groupId}/members](/graph/api/group-list-members) | When `type` is `person` then `/microsoft.graph.user` will be appended to the request path, a `$filter` parameter is composed with the user input value |
| `group-id` is not set and `type` set to `person` or `any` and `user-type` set to `any` and `group-ids` is set and `transitive-search` is true | GroupMember.Read.All, Group.Read.All, Directory.Read.All, GroupMember.ReadWrite.All, Group.ReadWrite.All  | [/groups/${groupId}/transitiveMembers](/graph/api/group-list-transitivemembers)  | When `type` is `person` then `/microsoft.graph.user` will be appended to the request path, a `$filter` parameter is composed with the user input value |
| neither `group-id` nor `group-ids` is set `type` set to `person` or `any` and `user-type` is set to `any`                                     | People.Read, People.Read.All                                                                              | [/me/people](/graph/api/user-list-people) | When `people-filters` is set or `user-type` is not `any` a $filter parameter is added to the request. If `user-type` is not `contact` the `X-PeopleQuery-QuerySources: 'Mailbox,Directory'` header is set on the request |
| `type` set to `person` or `any` and `user-type` not set to `any` and `user-ids` is not set                                                    | User.ReadBasic.All, User.Read.All, Directory.Read.All, User.ReadWrite.All, Directory.ReadWrite.All        | [/users](/graph/api/user-list) | User input is used to build a `$search` parameter. When `user-filters` is set this is added as the $filter parameter to the request with `$count=true` and the `ConsistencyLevel: 'eventual'` header is set on the request |
| `type` set to `person` or `any` and `user-type` not set to `any` and `user-ids` is set                                                        | User.ReadBasic.All, User.Read.All, Directory.Read.All, User.ReadWrite.All, Directory.ReadWrite.All        | [/users/$({userId}](/graph/api/user-get) | When `user-filters` is set this is added as the $filter parameter to the request with `$count=true` and the `ConsistencyLevel: 'eventual'` header is set on the request |
| `type` set to `person` or `any` and `user-type` set to `any` and `group-ids` is not set and `user-ids` is set                                 | User.ReadBasic.All, User.Read.All, Directory.Read.All, User.ReadWrite.All, Directory.ReadWrite.All        | [/users/$({userId}](/graph/api/user-get) | When `user-filters` is set this is added as the $filter parameter to the request with `$count=true` and the `ConsistencyLevel: 'eventual'` header is set on the request |
| `group-id` is not set and `type` is `group` or `type` is `any` and fewer results than `show-max` were found                                   | GroupMember.Read.All, Group.Read.All, Directory.Read.All, Group.ReadWrite.All, Directory.ReadWrite.All    | [/groups](/graph/api/group-list)  |  A `$filter` is composed using the supplied user input, `group-filters`, and `group-type` values   |
| `group-id` is not set and `group-ids` is set and `type` is `group` or `type` is `any` and fewer results than `show-max` were found            | GroupMember.Read.All, Group.Read.All, Directory.Read.All, Group.ReadWrite.All, Directory.ReadWrite.All    | [/groups](/graph/api/group-list)  |  A `$filter` is composed using the supplied user input, `user-filters`, and `group-type` values   |
