...
This component might make many queries depending on configuration and state. The Microsoft Graph APIs and permissions required are split into three sections here for easier understanding. For each API called, the user must have at least one of the listed permissions.
