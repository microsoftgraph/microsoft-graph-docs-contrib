...
| user-filters | userFilters | Specifies the filter criterias to use when querying the users endpoint. It requires the `user-type` to be set to `user` or `contact`. By default, the `user-type` is `any` and this leads the querying to take place in the `people` endpoint block. Example: `user-filters="startsWith(displayName,'a')"`. This attribute is optional. More information on the support for filter on user properties of Azure AD directory objects can be found [here](https://docs.microsoft.com/en-us/graph/aad-advanced-queries?tabs=http#user-properties).
| group-filters | groupFilters | Specifies the filter criterias to use when querying the `groups` endpoint. It requires the `type` to be set to `group`. Example: `group-filters="startsWith(displayName,'a')"`. This attribute is optional.
| people-filters | peopleFilters | Specifies the filter criterias to use when querying the `people` endpoint. It is used as it is. Example: `people-filters="jobTitle eq 'Web Marketing Manager'"`. This attribute is optional. More information about filtering and the supported capabilities on the people resource can be found [here](https://docs.microsoft.com/en-us/graph/people-example).
