...
The Microsoft Graph Toolkit React components (`mgt-react` and `mgt-chat`) allow React developers to use the Microsoft Graph Toolkit in their React applications. The library wraps all Microsoft Graph Toolkit components and exports them as React components.
...
In addition to the core components, it's also possible to use both `mgt-chat` and `mgt-new-chat` components in React applications. This requires a separate installation.
...
If you are using `mgt-chat`, import the components separately:
...
Most Microsoft Graph Toolkit components [support templating](../customize-components/templates.md) and `mgt-react` allows you to use React to write templates. In the case of the chat components, they don't support custom templating.
