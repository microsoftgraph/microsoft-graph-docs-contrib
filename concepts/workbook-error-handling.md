...
| **tooManyRequestsUncategorized**             | The failed request exceeds certain frequency limitation. The Microsoft Graph client is not expected to resend the failed request until the specified cooldown duration passes. Also refer to [Throttling](workbook-best-practice.md#throttling) for best practices to reduce throttling.
