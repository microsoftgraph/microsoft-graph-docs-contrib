...
> [!NOTE]
> [RFC 3986](https://www.rfc-editor.org/rfc/rfc3986#section-2.2) requires that reserved characters are percent encoded in URLs. While the URLs in this document don't use encoded reserved characters, for example, the comma (`,`) in a `startsWith` expression, these characters must be percent encoded in your requests.
