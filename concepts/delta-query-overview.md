...
>**Note:** The Delta Query function is generally referred to by appending `/delta` to the resource name. However `/delta` is a shortcut for the fully qualified name `/microsoft.graph.delta` that developers might see when looking at requests generated by the Microsoft Graph SDKs.
