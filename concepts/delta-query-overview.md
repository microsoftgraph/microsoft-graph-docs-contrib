...
#### Token duration
Delta tokens are only valid for a specific period before the client application needs to run a full synchronisation again. For identity objects (`directoryObject`, `directoryRole`, `group`, `user`) the limit is 30 days.
