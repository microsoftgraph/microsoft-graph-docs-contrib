...
> **Note**: When the user hides a chat from the Teams client, they get a notification with `isHidden: true` in the **viewpoint** property. But there is no notification with `isHidden: false` when the chat becomes visible again in Teams client after a new message arrives in the chat for the user. The subscriber has to look at the `lastMessageReadDateTime` in the **viewpoint** property and the `CreatedDateTime` of the new message to figure out that the chat is not hidden for the user **(subscriber must have active subscription for [receiving change notification](https://learn.microsoft.com/en-us/graph/teams-changenotifications-chatmessage) for new message for this chat and know when hidden chat has new message)**. When the user opens this unhidden chat from Teams client and reads the newly arrived message, a change notification will finally be sent with `isHidden: false` and with updated `lastMessageReadDateTime` for the **viewpoint** property.
