...
> **Note**: When the user hides a chat from the Teams client, they get a notification with `isHidden: true` in the **viewpoint** property. But there is no notification with `isHidden: false` when the chat becomes visible again in the Teams client after a new message arrives in the chat for the user. The subscriber has to look at the `lastMessageReadDateTime` in the **viewpoint** property and compare it to the `createdDateTime` of the new message - if the `createdDateTime` of the message comes after the `lastMessageReadDateTime`, then the chat is no longer hidden for the user. **The subscriber must have an active subscription to [receive change notifications](https://learn.microsoft.com/graph/teams-changenotifications-chatmessage) about messages in the chat to be notified when the hidden chat has new messages**. Finally, when the user opens this unhidden chat from the Teams client and reads the newly arrived message, a change notification will be sent with `isHidden: false` and with an updated `lastMessageReadDateTime` for the **viewpoint** property.
