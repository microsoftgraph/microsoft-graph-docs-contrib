...
>**Important:** If you already have an app in US Government and you're using the `https://graph.microsoft.com` endpoint, we recommend that you should switch to the new `https://graph.microsoft.us` endpoint. The `https://graph.microsoft.com` is currently functional but will be disabled in near future.
