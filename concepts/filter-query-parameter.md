...
<sup><a name=":hash:"></a> :hash: </sup> : This example is only supported with [advanced query capabilities](/graph/aad-advanced-queries).
...
| `not`                  | `~/users?$filter=not(userType eq 'Member')`<sup>[:hash:]</sup></sup>                                                                                       |
| `ne`                   | `~/users?$filter=companyName ne null`<sup>[:hash:]</sup></sup>                                                                                             |
| `startsWith`           | `~/users?$filter=startsWith(userPrincipalName, 'admin')`                                                                           |
| `endsWith`             | `~/users?$filter=endsWith(mail,'@outlook.com')`<sup>[:hash:]</sup></sup>                                                                                   |
...
| `le`                   | `~/devices?$filter=registrationDateTime le 2021-01-02T12:00:00Z`<sup>[:hash:]</sup></sup>                                                                  |
| `ge`                   | `~/devices?$filter=registrationDateTime ge 2021-01-02T12:00:00Z`<sup>[:hash:]</sup></sup>                                                                  |
| `not` and `endsWith`   | `~/users?$filter=not(endsWith(mail, 'OnMicrosoft.com'))`<sup>[:hash:]</sup></sup>                                                                          |
| `not` and `startsWith` | `~/users?$filter=not(startsWith(mail, 'A'))`<sup>[:hash:]</sup></sup>                                                                                      |
| `not` and `eq`         | `~/users?$filter=not(companyName eq 'Contoso E.A.')`<sup>[:hash:]</sup></sup>                                                                              |
| `not` and `in`         | `~/users?$filter=not(userType in ('Member'))`<sup>[:hash:]</sup></sup>                                                                                     |
...
| `not`                                    | `~/groups?$filter=not(groupTypes/any(c:c eq 'Unified'))`<sup>[:hash:]</sup></sup>          |
| `ne`                                     | `~/users?$filter=companyName ne null`<sup>[:hash:]</sup></sup>                             |
| `startsWith`                             | `~/users?$filter=businessPhones/any(p:startsWith(p, '44'))`<sup>[:hash:]</sup></sup>       |
| `endsWith`                               | `~/users?$filter=endsWith(mail,'@outlook.com')`<sup>[:hash:]</sup></sup>                   |
| `not` and `endsWith`                     | `~/groups?$filter=not(endsWith(mail,'OnMicrosoft.com'))`<sup>[:hash:]</sup></sup>          |
| `not` and `startsWith`                   | `~/groups?$filter=not(startsWith(mail,'Pineview'))`<sup>[:hash:]</sup></sup>               |
| `not` and `eq`                           | `~/groups?$filter=not(mail eq 'PineviewSchoolStaff@Contoso.com')`<sup>[:hash:]</sup></sup> |
| `eq` and `$count` for empty collections  | `~/users?$filter=assignedLicenses/$count eq 0`<sup>[:hash:]</sup></sup>                    |
| `ne` and `$count` for empty collections  | `~/users?$filter=assignedLicenses/$count ne 0`<sup>[:hash:]</sup></sup>                    |
| `not` and `$count` for empty collections | `~/users?$filter=not(assignedLicenses/$count eq 0)`<sup>[:hash:]</sup></sup>               |
| `$count` for collections with one object | `~/servicePrincipals?$filter=owners/$count eq 1`<sup>[:hash:]</sup></sup>                  |
...
| Operator (s) | Syntax                                                                                                         |
|---------------|-----------------------------------------------------------------------------------------------------------------|
| `eq`         | `~/servicePrincipals?$filter=appOwnerOrganizationId eq 72f988bf-86f1-41af-91ab-2d7cd011db47`<sup>[:hash:]</sup></sup>                  |
| `not`        | `~/servicePrincipals?$filter=not(appOwnerOrganizationId eq 72f988bf-86f1-41af-91ab-2d7cd011db47)`<sup>[:hash:]</sup></sup> |
...
| Operator (s) | Syntax                                                         |
|---------------|-----------------------------------------------------------------|
| `eq`         | `~/devices?$filter=alternativeSecurityIds/any(a:a/type eq 2)`<sup>[:hash:]</sup></sup> |
| `le`         | `~/devices?$filter=alternativeSecurityIds/any(a:a/type le 2)`<sup>[:hash:]</sup></sup> |
| `ge`         | `~/devices?$filter=alternativeSecurityIds/any(a:a/type ge 2)`<sup>[:hash:]</sup></sup> |
...
| Operator (s)   | Syntax                                                                                    |
|-----------------|--------------------------------------------------------------------------------------------|
| `eq`           | `~/users?$filter=authorizationInfo/certificateUserIds/any(x:x eq '9876543210@mil')`<sup>[:hash:]</sup></sup>      |
| `not` and `eq` | `~/users?$filter=not(authorizationInfo/certificateUserIds/any(x:x eq '9876543210@mil'))`<sup>[:hash:]</sup></sup> |
| `startsWith`   | `~/users?$filter=authorizationInfo/certificateUserIds/any(x:startswith(x,'987654321'))`<sup>[:hash:]</sup></sup>  |
| `endsWith`     | `~/users?$filter=proxyAddresses/any(p:endsWith(p,'OnMicrosoft.com'))`<sup>[:hash:]</sup></sup>                    |
