...
> [!WARNING]
> Don't store secrets or other sensitive information—such as passwords, credentials, access or refresh tokens, or connection strings—in directory object attributes or any other resource that Microsoft Graph exposes. Microsoft Graph activity logs, along with other audit and sign-in logs, capture details about API requests, including request URIs. Any sensitive value that's passed in a request or stored on an auditable object can be recorded in these logs, where it's readable by tenant administrators and by any downstream Log Analytics workspace, Azure Storage account, or SIEM system that the logs are exported to. Directory attributes aren't a secure store for secrets; use a dedicated secret store such as [Azure Key Vault](/azure/key-vault/general/overview) instead.
