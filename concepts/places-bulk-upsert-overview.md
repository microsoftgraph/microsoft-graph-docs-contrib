...
The [bulk upsert](/graph/api/place-patch-places?view=graph-rest-beta&preserve-view=true) API allows you to efficiently create and update multiple [places](/graph/api/resources/place?view=graph-rest-beta&preserve-view=true) in a single asynchronous operation.

## Supported scenarios

The following scenarios are supported when using the **bulk upsert** API:
...
Job-level concurrency limits the number of bulk upsert operations that can run simultaneously within a tenant.
