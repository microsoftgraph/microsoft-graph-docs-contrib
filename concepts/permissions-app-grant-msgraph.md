---
title: "Grant API app roles using Microsoft Graph"
...
# Grant API app roles using Microsoft Graph
...
Use the following instructions to grant app roles that are exposed by an API to an app. App roles, also called application permissions, app-only permissions, or direct access permissions, allow an app to call an API with its own identity.
...
1. A working Azure AD tenant, and you must complete the following steps:
    ...
    3. Get the object ID of a resource service principal that exposes app roles, and the ID of the app role to grant. This article uses the Microsoft Graph service principal in the tenant as the resource service principal.
...
In this step, you'll assign your app an app role that's exposed by Microsoft Graph, thereby creating an app role assignment. In the following example, the object ID of Microsoft Graph in this tenant is `943603e4-e787-4fe9-93d1-e30f749aae3`. The app role `User.Read.All` is identified by ID `df021288-bdef-4463-88db-98f22de89214`.

Therefore, you'll grant your client app (the principal of ID `ef969797-201d-4f6b-960c-e9ed5f31dab5`) an app role of ID `df021288-bdef-4463-88db-98f22de89214` that's exposed by a resource service principal of ID `943603e4-e787-4fe9-93d1-e30f749aae3`.
...
You've granted an app role to a service principal. This method of granting permissions using Microsoft Graph is an alternative [interactive consent](/azure/active-directory/develop/application-consent-experience) and should be used with caution.
