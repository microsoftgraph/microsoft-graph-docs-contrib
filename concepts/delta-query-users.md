...
- When the user is [permanently deleted](/graph/api/directory-deleteditems-delete), the item contains an annotation: `@removed` with value of `"reason": "deleted"`.
...
Using the `deltatoken` from the [last response](#final-nextlink-response), you get changes (additions, deletions, or updates) to users since the last request.
