...
| [health monitoring alert][] | Changes to all health monitoring alerts: `reports/healthmonitoring/alerts` <br><br> Changes to a specific type of alert: `reports/healthmonitoring/alert` with the `notificationQueryOptions` property in request payload set as `$filter=alertType eq '{alertType}'` |-|
