...
In Microsoft Graph, users are represented by the [user](/graph/api/resources/user) resource type. The following table lists common operations that you can perform on users.

| Path | Description |
|--|--|
| [`/me`](/graph/api/user-get) | Get the signed-in user's details. |
| [`/users`](/graph/api/user-list) | Lists users in the organization. |
| [`/users/{id}`](/graph/api/user-get) | Gets a specific user by ID. |
| [`/users/{id}/photo/$value`](/graph/api/profilephoto-get) | Gets the user's profile photo. |
| [`/users/{id}/manager`](/graph/api/user-list-manager) | Gets the user's manager. |
| [`/users/{id}/messages`](/graph/api/user-list-messages) | Lists the user's email messages in their primary inbox. |
| [`/users/{id}/events`](/graph/api/user-list-events) | Lists the user's upcoming events in their calendar. |
| [`/users/{id}/drive`](/graph/api/drive-get) | Gets the user's OneDrive file store. |
| [`/users/{id}/memberOf`](/graph/api/user-list-memberof) | Lists the groups that the user is a member of. |
| [`/users/{id}/joinedTeams`](/graph/api/user-list-joinedteams) | Lists the Microsoft Teams that the user is a member of. |
| [`POST /invitations`](/graph/api/invitation-post) | Invite a guest as part of business-to-business (B2B) collaboration in Microsoft Entra External ID |

## Authorization and privileges

Microsoft Graph supports [delegated and application permissions](/graph/permissions-overview#permission-types) to manage user operations. Understanding these permissions is crucial for secure and efficient application development. 
...
Other operations, including managing details for other users, require administrative privileges granted through Microsoft Graph permissions that only authorized administrators can consent to, and [Microsoft Entra roles](/entra/identity/role-based-access-control/permissions-reference?toc=%2Fgraph%2Ftoc.json). In addition, some operations are considered sensitive and only limited administrators can perform them. For more information, see [Who can reset passwords](#who-can-reset-passwords) and [Who can update sensitive attributes](#who-can-perform-sensitive-actions) sections.
...
The following represent the default set of properties that are returned when getting a user or listing users in v1.0. These are a subset of all available properties in v1.0. To get more user properties, use the `$select` query parameter. Learn [how to use the $select query parameter](/graph/query-parameters#select-parameter) and see [properties that support the $select query parameter](/graph/api/resources/user#properties).

For details and a list of all properties, see the [user](/graph/api/resources/user) object.
...
For details and a list of all the properties, see the [user](/graph/api/resources/user) object.
...
Some properties of the user object aren't returned by default and must be specified in a `$select` query parameter. For example, **birthday** and **skills**. Refer to the [properties table of the user entity](/graph/api/resources/user#properties) to identify properties that are returned only when you `$select`.
