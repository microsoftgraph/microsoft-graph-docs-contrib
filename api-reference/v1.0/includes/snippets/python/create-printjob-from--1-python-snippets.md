...
configuration.@odata_type = 'microsoft.graph.printJobConfiguration'
...
page_ranges_integer_range1.@odata_type = 'microsoft.graph.integerRange'
...
result = await client.print.printers.by_printer_id('printer-id').jobs.post(request_body = request_body)
