...
request_body.@odata_type = '#microsoft.graph.deviceConfigurationDeviceStatus'
...
result = await client.device_management.device_configurations.by_device_configuration_id('deviceConfiguration-id').device_statuses.by_device_statuse_id('deviceConfigurationDeviceStatus-id').patch(request_body = request_body)
