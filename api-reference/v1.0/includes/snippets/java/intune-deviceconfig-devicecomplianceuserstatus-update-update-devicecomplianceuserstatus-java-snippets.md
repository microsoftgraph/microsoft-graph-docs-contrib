...
DeviceComplianceUserStatus result = graphClient.deviceManagement().deviceCompliancePolicies().byDeviceCompliancePolicyId("{deviceCompliancePolicy-id}").userStatuses().byDeviceComplianceUserStatusId("{deviceComplianceUserStatus-id}").patch(deviceComplianceUserStatus);
