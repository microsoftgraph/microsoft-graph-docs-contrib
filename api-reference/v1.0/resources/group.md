...
|groupTypes|String collection| Specifies the group type and its membership.  <br><br>If the collection contains `Unified` then the group is a Microsoft 365 group; otherwise it's either a security group or distribution group. Refer to [groups overview](groups-overview.md) for more details.  <br><br>If the collection includes `DynamicMembership`, the group has dynamic membership; otherwise, membership is static.  <br><br>Returned by default. Supports $filter.|
