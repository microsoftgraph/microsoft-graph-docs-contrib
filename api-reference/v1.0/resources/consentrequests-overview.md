...
The Azure AD consent workflow allows users to send requests for the admins to grant consent to apps that require admin approval. The consent requests APIs in Microsoft Graph allows admin to configure the consent workflow and track consent requests for both apps and users.
...
Use the [adminConsentRequestPolicy](../resources/adminconsentrequestpolicy.md) resource type and its associated methods to configure the admin consent workflow as follows:
...
When a user requests for an app to be authorized for use in the tenant or for the app to be granted delegated permissions on their behalf, they create an app consent request. App consent requests are retrieved through the [appConsentRequest](../resources/appconsentrequest.md) resource type and its associated methods.
...
+ Retrieve the delegated permissions associated with the app consent request.
+ User `$filter` to match against pending requests.
...
When a user requests the admin to authorize an app for use in the tenant or for the app to be granted delegated permissions on their behalf, their request is a user consent request. User consent requests are retrieved through the [userConsentRequest](../resources/userconsentrequest.md) resource type and its associated methods.
