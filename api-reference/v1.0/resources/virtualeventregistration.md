...
|status|microsoft.graph.virtualEventAttendeeRegistrationStatus|Registration status of the registrant. Read-only. Possible values are `registered`, `canceled`, `whitelisted`, `pendingApproval`, `rejectedByOrganizer`, and `unknownFutureValue`. |
