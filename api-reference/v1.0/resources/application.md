...
| tags |String collection| Custom strings that can be used to categorize and identify the application. Not nullable. Strings are merged into the [service principal](serviceprincipal.md) tags property.<br><br>Supports `$filter` (`eq`, `not`, `ge`, `le`, `startsWith`).|
