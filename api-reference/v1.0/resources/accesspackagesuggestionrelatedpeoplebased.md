...
Represents access package suggestions that are based on related people insights in [Microsoft Entra entitlement management](../resources/entitlementmanagement-overview.md). This suggestion type includes information about related users who may have access to similar resources, helping to provide contextual suggestions to end users.
...
|:-------------|:------------|:------------|
|relatedPeople|Collection([identity](../resources/identity.md))|A collection of identities representing people related to the current user who may have access to similar resources. This property is only populated when the tenant's endUserSettings have `relatedPeopleInsightLevel` set to `countAndNames`. This includes both the user ID and display name information.|
