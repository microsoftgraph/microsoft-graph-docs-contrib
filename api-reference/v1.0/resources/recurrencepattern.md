...
> **Note** Include only the properties that you need for a recurrence pattern. Any property that you include that does not have a supported value would return an error.
...
| Weekly | `weekly` | Event repeats on the same day or days of the week, based on the number of weeks between each set of occurrences. | Repeat event every other Monday and Tuesday. | **type**, **interval**, **daysOfWeek**, **firstDayOfWeek** |
