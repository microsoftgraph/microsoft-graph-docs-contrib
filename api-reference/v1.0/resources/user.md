...
| createdDateTime | DateTimeOffset | The date and time, in ISO 8601 format and in UTC time, when the user was created. Property is `null` for users created before June 2018 and on-premises users that were synced to Azure AD before June 2018. A `null` value indicates that an accurate creation time couldn't be determined for the user. Read-only. <br><br>Returned only on `$select`. Supports `$filter` (`eq`, `ne`, `not` , `ge`, `le`, `in`).|
