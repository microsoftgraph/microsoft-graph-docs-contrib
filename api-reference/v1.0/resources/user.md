...
|:---------------|:--------|:----------|
|aboutMe|String|A freeform text entry field for the user to describe themselves. <br>Returned only on `$select` on the Get user API (`GET /users/{ID}` or `/me`).|
...
|birthday|DateTimeOffset|The birthday of the user. The Timestamp type represents date and time information using ISO 8601 format and is always in UTC time. For example, midnight UTC on Jan 1, 2014 is `2014-01-01T00:00:00Z`. <br>Returned only on `$select` on the Get user API (`GET /users/{ID}` or `/me`). |
