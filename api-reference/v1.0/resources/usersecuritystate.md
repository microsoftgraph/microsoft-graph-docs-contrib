...
|emailRole|emailRole|For email-related alerts - user account's email 'role'. The possible values are: `unknown`, `sender`, `recipient`.|
...
|logonType|logonType|Method of user sign in. The possible values are: `unknown`, `interactive`, `remoteInteractive`, `network`, `batch`, `service`.|
...
|userAccountType|userAccountSecurityType|User account type (group membership), per Windows definition. The possible values are: `unknown`, `standard`, `power`, `administrator`.|
