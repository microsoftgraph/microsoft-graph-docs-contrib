...
|:---------|:-----|:------------|
| changeType | [changeType](changetype.md) | Indicates the type of change that will raise the change notification. The supported values are: `created`, `updated`, `deleted`. Required. |
...
| lifecycleEvent | [lifecycleEventType](lifecycleeventtype.md) | The type of lifecycle notification if the current notification is a lifecycle notification. Optional. Supported values are `missed`, `removed`, `reauthorizationRequired`. |
