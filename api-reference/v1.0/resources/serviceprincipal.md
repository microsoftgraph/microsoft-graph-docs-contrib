...
|preferredTokenSigningKeyThumbprint|String|This property can be used on SAML applications (apps that have preferredSingleSignOnMode set to `saml`) to control which certificate is used to sign the SAML responses. For applications that are not SAML (apps that have preferredSingleSignOnMode set to anything other than `saml`), do not write or otherwise rely on this property.|
