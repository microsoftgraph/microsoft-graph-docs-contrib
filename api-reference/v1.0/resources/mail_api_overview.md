# Using Microsoft Graph API to integrate with Outlook mail
...
You can use well-known folder names such as `Inbox`, `Drafts`, `SentItems`, `DeletedItems` to identify certain mail folders that exist by default for all users.
As an example, you can get messages in the Outlook **Sent Items** folder of the signed-in user's, without first getting the folder ID:
```
...
Microsoft Graph API also provides methods and actions that support common use cases of messages, as listed below. 
