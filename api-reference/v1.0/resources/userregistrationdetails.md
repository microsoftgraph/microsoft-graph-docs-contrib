...
| `isSystemPreferredAuthenticationMethodEnabled`  | `Edm.Boolean`                                         | Illustrates if the system will dynamically determine most secure authentication method user is registered with for secondary authentication. | Yes       | Yes      |
| `systemPreferredAuthenticationMethods`        | `Collection(string)`                      | This is the list of values that the system determined to be the most secure authentication methods among the registered methods for second factor authentication. The value will be an empty array if `isSystemPreferredAuthenticationMethodEnabled` is false  | Yes      | Yes      |
| `userPreferredMethodForSecondaryAuthentication`        | `microsoft.graph.userDefaultAuthenticationMethodType`                      | The channel of MFA communication preferred by this user for secondary authentication. The property is used as preferred MFA method when `isSystemPreferredAuthenticationMethodEnabled` is false.  | Yes      | Yes      |
| `lastUpdatedDateTime`        | `dateTimeOffset`                      | The datetime representing the last time the record was updated                                                                                                                             | Yes      | Yes      |
