...
If successful, this method returns a `200 OK` response code and collection of **alert** objects in the response body. If an status code other than 2xx or 404 is returned from a provider or when the provider times out, then the response will be a 206 status code with the providers response in a warning header. Read [Microsoft Graph Security API error responses](../resources/security-error-codes.md) for additional information.
