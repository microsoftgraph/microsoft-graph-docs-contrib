...
> **Note:** The `$filter` parameter is required, along with the read all permissions of each type specified. For example, for `https://graph.microsoft.com/v1.0/directoryObjects/delta/?$filter=isof('microsoft.graph.user') or isof('microsoft.graph.application')'`, both User.Read.All and Application.Read.All are required. 
