...
| `LocalizationSchemaIsMissing` |The "schema" property is missing in the Localization file.| The localization file is missing the "schema" property.|
| `LocalizationSchemaDoesNotFollowConventionalSchemaRegex` |Localization file does not use the correct schema URL: developer.microsoft.jsonschema URL. | The schema URL in the localization file does not match the expected schema format.|
| `LocalizationSchemaVersionIsNotTheSameAsTheManifestSchemaVersion` |The schema version in the Localization file does not match the version in the app manifest.| The schema version in the localization file does not match the version specified in the app manifest.|
...
| `ValidDomainsContainsUrl` |The list of valid domains for an app must include a URL.| The domains specified in the localization file must be valid.|
