...
| managerApplications | Guid collection | A collection of application IDs for Microsoft first-party applications designated as managers. This property is supported only on [agentIdentityBlueprint](../resources/agentidentityblueprint.md) objects and isn't updated by using this **Update application** API. To update **managerApplications**, use the dedicated **Update agentIdentityBlueprint** API. Updating **managerApplications** on non-agent-blueprint applications returns a `400 Bad Request` error. Only Microsoft first-party application IDs can be added as managers; adding a third-party application returns a `400 Bad Request` error. The maximum allowed is 10 manager applications; exceeding this limit returns a `400 Bad Request` error. |
