...
| $filter  | `$filer` is **required** in the request URL of this API. The API currently only supports filtering by one field in a call. For details, see [Supported properties with $filter](#supported-properties-with-filter).           |
...
| Property    | Example                                   
| :---------- | :----------------------------------------- |
| recordType       | `$filer=recordType eq 'A'`          |
