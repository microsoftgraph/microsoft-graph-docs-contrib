...
Retrieve the properties and relationships of an [securescorecontrolprofiles](../resources/securescorecontrolprofile.md) object.
...
If successful, this method returns a `200 OK` response code and an **secureScoreControlProfiles** object in the response body. If a status code other than 2xx or 404 is returned from a provider or if a provider times out, the response will be a `206 Partial Content` status code with the provider's response in a warning header. For more information, see [Microsoft Graph Security API error responses](../resources/security-error-codes.md).
