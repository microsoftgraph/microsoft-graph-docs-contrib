...
> [!NOTE]
> Currently, installing an app that requires resource-specific consent (RSC) permissions is not supported in the application context. If you install an app with RSC permission, it shows an error `412 - Precondition Failed` with `Failed to execute TeamsGraphService backend request IsUserAuthorizedToGrantGroupResourceSpecificPermissionsRequest. Workload Unknown`.
