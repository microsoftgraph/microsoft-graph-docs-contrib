...
|[cancel](../api/approvalitem-cancel.md)|None|Cancel is used for cancelling the approval item. This endpoint would be triggered by the owner of the approval item.|
...
|state|approvalItemState|This is the approvalItem state.The possible values are: `canceled`, `created`, `pending`, `completed`, `unknownFutureValue`. Read-only.|
