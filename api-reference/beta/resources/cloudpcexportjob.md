...
|remoteConnectionQualityReports (deprecated)|The overall connection quality report for all devices within a tenant. The `remoteConnectionQualityReports` member is deprecated and no longer supported. Use the `remoteConnectionQualityReport` member instead.|
...
|inaccessibleCloudPcTrendReport|The daily aggregated report for a specified period that contains details of Cloud PCs that are inaccessible, including Cloud PCs with consecutive connection failures or that are in an unavailable state.|
