...
|dnsCheckNameWithInvalidCharacter|The DNS check failed because the domain name entered contains an illegal character. Please make sure the domain name only contains legal characters.|
...
|azureAdDeviceSyncCheckScpNotConfigured|Hybrid Azure AD join failed due to SCP misconfiguration. Make sure your Service Configuration Point (SCP) configuration is valid and available to perform a Hybrid Azure AD join. Your SCP can be created and configured in the Azure AD Connect wizard.|
