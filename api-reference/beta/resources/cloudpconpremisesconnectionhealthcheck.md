...
|adJoinCheckFqdnNotFound|While attempting domain join, the domain name couldn't be found. Please make sure a domain controller for the domain name can be contacted by the vNet defined in the on-premises network connection.|
|adJoinCheckIncorrectCredentials|While attempting domain join, credentials provided for the domain aren't correct. Please update the on-premises network connection with correct credentials.|
|adJoinCheckOrganizationalUnitNotFound|While attempting domain join, the organizational unit (OU) can't be found. Please provide an OU in the domain. The OU must be in Distinguished Name format. Example format: “OU=OU1,OU=OU2,OU=OU3,DC=DC1”.|
|adJoinCheckOrganizationalUnitIncorrectFormat|While attempting domain join, the organizational unit (OU) can't be found. Please provide an OU in the domain. The OU must be in Distinguished Name format. Example format: “OU=OU1,OU=OU2,OU=OU3,DC=DC1”.|
...
|resourceAvailabilityCheckAzurePolicyViolation|The Azure subscription provided can't be found. Please ensure the Azure subscription is available for provisioning.|
...
|resourceAvailabilityCheckUnknownError|An unknown error occurred while checking Azure resources. Please ensure that all Azure resources meet the prerequisites.|
