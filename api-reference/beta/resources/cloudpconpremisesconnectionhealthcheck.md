...
|udpConnectivityCheckStunUrlNotAllowListed|Your current network configuration does not allow the use of UDP direct connect (STUN, Session Traversal Utilities for NAT). This will not prevent the use of Cloud PCs but may prevent optimal performance. Consider your own network configuration policies before making changes.|
