...
|status|microsoft.graph.networkaccess.tlsCertificateStatus|The current status of the certificate. The possible values are: `csrGenerated`, `enrolling`, `active`, `expiring`, `expired`, `enabled`, `disabled`, `unknownFutureValue`. Read-only. Supports `$filter` (`eq`, `ne`)|
