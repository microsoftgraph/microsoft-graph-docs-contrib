...
|isManagementRestricted|Boolean|`true` if the device should be treated as sensitive, which requires specific permissions to manage. Default value is `false`. Read-only.|
