...
| extensionAttributes | [extensionAttributes](extensionattributes.md) | Contains extensionAttributes 1-15 for the device. The individual extension attributes are not selectable. These properties may be set during creation or update of a device in Azure AD. <br><br>Returned only on `$select`. Supports `$filter` (`eq`, `NOT`, `ge`, `le`, `in`). |
