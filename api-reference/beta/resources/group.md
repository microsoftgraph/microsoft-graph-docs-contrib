...
|hideFromAddressLists |Boolean |Indicates if the group is hidden in part of the Outlook user interface: in the **Address Book**, in address lists for selecting message recipients, and in the **Browse Groups** dialog for searching groups. Default value is **false**. <br><br>Returned only on $select.|
|hideFromOutlookClients |Boolean |Indicates if the group is hidden in Outlook clients, such as Outlook for Windows and Outlook on the web. Default value is **false**. <br><br>Returned only on $select.|
