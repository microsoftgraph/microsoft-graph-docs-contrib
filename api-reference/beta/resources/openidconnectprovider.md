...
|clientSecret|String|The client secret for the application obtained when registering the application with the identity provider. <br />The clientSecret has a dependency on **responseType**. <ul><li>In the case of **responseType** = `code`, a secret is required for the auth code exchange.</li><li>In the case of **responseType** = `id_token` it is not required because there is no code exchange, the id_token is returned directly from the authorization response.</li></ul><br />This is write-only. A read operation will return `****`. Inherited from [identityProvider](../resources/identityprovider.md).|
