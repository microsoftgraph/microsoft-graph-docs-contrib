...
|:---|:---|:---|
|matchOn|matchOn|The matchOn property identifies the group attribute on which to apply the filter. The possible values are: `displayName`, `samAccountName`, `unknownFutureValue`.|
|type|filterType|The type property selects the type of filter you wish to apply to the attribute selected by the MatchOn property. The possible values are: `prefix`, `suffix`, `contains`, `unknownFutureValue`.|
