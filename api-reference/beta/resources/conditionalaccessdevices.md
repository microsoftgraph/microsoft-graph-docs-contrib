...
| deviceFilter | [conditionalAccessFilter](conditionalaccessfilter.md) | Filter defining the dynamic-device-syntax rule to include/exclude devices. A filter can use device properties (e.g. extension attributes) to include/exclude them. Cannot be set if *includeDevices* or *excludeDevices* is set. |
