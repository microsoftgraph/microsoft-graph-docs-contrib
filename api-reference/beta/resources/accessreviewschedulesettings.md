...
| recurrence|[patternedRecurrence](../resources/patternedrecurrence.md) | Detailed settings for recurrence using standard Outlook recurrence object. Note that only `weekly` and `absoluteMonthly` on **recurrencePattern** are supported. Use property **startDate** on **recurrenceRange** to determine the day the review will start on. |
