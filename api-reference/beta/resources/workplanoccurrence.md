...
Only time-off occurrences can be deleted. Occurrences generated from recurrences can't be deleted. Only time-off occurrences can be created directly. Other occurrences are autogenerated from recurrences. Updates require the full occurrence object to be provided (PUT semantics). PATCH isn't supported. Reading individual occurrences by ID isn't supported. Use the occurrencesView function with date range parameters instead.
