...
|status|cloudPcBulkActionStatus|The status of the bulk action. Possible values are: `pending`, `succeeded`, `failed`, `unknownFutureValue`. The default value is `pending`. Read-only. Inherited from [cloudPcBulkAction](../resources/cloudpcbulkaction.md).|
