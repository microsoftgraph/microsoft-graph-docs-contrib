...
|isContinuousAccessEvaluationEnabled|Boolean| Indicates whether [Continuous access evaluation](entra/identity/conditional-access/concept-continuous-access-evaluation) is enabled for Applocation proxy application. For all Application Proxy apps, the property is set to `true` by default.|
