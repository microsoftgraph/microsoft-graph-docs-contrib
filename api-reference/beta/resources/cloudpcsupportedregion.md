...
|regionRestrictionDetail|[cloudPcSupportedRegionRestrictionDetail](../resources/cloudpcsupportedregionrestrictiondetail.md)| When the region is unavailable then all the region restrictions are set to 'true'.The restrictions can be enforced on three properties 'cPURestricted', 'gPURestricted', 'nestedVirtualizationRestrict'.'cPURestricted' means when the region is available for CPU; 'gPURestricted' means when the region is available for GPU; 'nestedVirtualizationRestricted' means when the region is available for Nested Virtualization. Read-only.|
...
``` json
{
  ...
  "regionRestrictionDetail": "microsoft.graph.cloudPcSupportedRegionRestrictionDetail"
}
```
