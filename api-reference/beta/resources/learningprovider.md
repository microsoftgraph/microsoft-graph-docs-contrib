...
|isCourseActivitySyncEnabled|Boolean|Indicates whether learning course activity records will be ingested for this provider or not. Default value is `false`. Learning course activities for a provider cannot be ingested if this value is false. This value needs to be updated to true when the provider wants to ingest learningCourseActivities.|
