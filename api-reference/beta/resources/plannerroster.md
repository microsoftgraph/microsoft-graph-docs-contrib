...
|:---|:---|:---|
| assignedSensitivityLabel | [sensitivityLabelAssignmentMethod](../resources/sensitivitylabelassignment.md#sensitivitylabelassignmentmethod-values)| The sensitivity label applied to the roster. If mandatory labeling is enabled for the user and no label is specified, the user can't create the roster. Similarly, if labels are mandatory for the user, the user can't change the roster's label to `null`. Possible values are: `standard`, `privileged`, `auto`, `unknownFutureValue`.|
