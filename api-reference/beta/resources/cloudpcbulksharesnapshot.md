---
title: "cloudPcBulkShareSnapshot resource type"
description: "Represents the entity that performs a bulk share snapshot action."
author: "doudoujinjin"
...
# cloudPcBulkShareSnapshot resource type

Namespace: microsoft.graph

[!INCLUDE [beta-disclaimer](../../includes/beta-disclaimer.md)]

Represents the entity that performs a bulk share snapshot action.
...
|restorePointDateTime|DateTimeOffset| The pivot date and time that is used to filter the snapshot by the snapshot taken time. The value cannot be modified once it is created. The Timestamp type represents date and time information using ISO 8601 format and is always in UTC time. For example, midnight UTC on Jan 1, 2014 would look like this: '2014-01-01T00:00:00Z'. Read-Only.
|restorePointTimeRange|restoreTimeRange| The comparator that is used to filter the restore point according to `retorePointTime`, value can be `before`, `after` or `beforeOrAfter`. The value cannot be modified once it is created. Read-Only.
|storageAccountId|String|The unique identifier for Secure Azure Storage Account, which will receive the restore points(snapshots). The value cannot be modified once it is created. Read-Only.
...
<!-- {
  "blockType": "resource",
  "keyProperty": "id",
  "@odata.type": "microsoft.graph.cloudPcBulkShareSnapshot",
