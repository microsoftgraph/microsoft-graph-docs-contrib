...
|averageFreezeDuration|Duration|Average for the received freeze duration related with video stream.|
...
|isAudioForwardErrorCorrectionUsed|Boolean|True if the forward error correction (FEC) was used at some point during session, false otherwise. The default value is null in the case that no information was registered.|
