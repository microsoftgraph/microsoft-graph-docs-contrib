...
| signInActivity | [signInActivity](signinactivity.md) | Get the last signed-in date and request ID of the sign-in for a given user.<br><br>Supports `$filter`, but not with any other filterable properties. <br><br>Returned only on `$select`. Read-only. <br>**Note:** AuditLogs.Read.All permissions are required to read this property. Details for this property require an Azure AD Premium P1/P2 license.|
