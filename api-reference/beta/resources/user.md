...
| country | String | The country/region in which the user is located; for example, "US" or "UK". Maximum length is 128 characters. <br><br>Returned only on `$select`. Supports `$filter` (`eq`, `ne`); and filter on `null` values only in eventual consistency. |
