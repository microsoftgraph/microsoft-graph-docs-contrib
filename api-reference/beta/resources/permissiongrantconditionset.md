...
| permissionType | permissionType | The permission type of the permission being granted. Possible values: <ul><li>`application` for application permissions (e.g. app roles), or </li><li>`delegated` for delegated permissions. </li><li>The value `delegatedUserConsentable` indicates delegated permissions which have not been configured by the API publisher to require admin consent—this value may be used in built-in permission grant policies, but cannot be used in custom permission grant policies.</li></ul> Required. |
