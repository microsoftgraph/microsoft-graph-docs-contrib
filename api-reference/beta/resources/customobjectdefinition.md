...
|fields|customObjectFieldDefinition collection|Field definitions for custom objects. Each field has required immutable **name** and required **dataType** properties. The possible **dataType** values are: `string`, `boolean`, `datetime`, `int64`, `double`, `unknownFutureValue`. Optional properties are **description**, **isCaseSensitive**, **isFilterable**, **isImmutable**, **isNullable**, **isRequired**, and **isUnique**. **isCaseSensitive** applies only to `string` fields. String values are normalized with Unicode Normalization Form C (NFC) and have leading and trailing Unicode whitespace removed; comparisons ignore casing unless **isCaseSensitive** is `true`. All Boolean properties default to `false` except **isNullable**, which defaults to `true`.|
