...
|flowType                   |attributeFlowType          |Defines when this attribute should be updated in the target directory. The possible values are: <br/><ul><li>`Always` (default) <br/></li><li>`ObjectAddOnly` - only when new object is created <br/></li><li> `MultiValueAddOnly` - only when the change is adding new values to a multi-valued attribute <br/></li><li> `ValueAddOnly` - If there is a current value, only flows "Add" operations; will not flow "Remove" operations  <br/></li><li> `AttributeAddOnly` - Only propagates changes if no current value exists at all <br><br> **Note:** AD2AAD provisioning jobs don't respect the `flowType` property value.|</li></ul>
