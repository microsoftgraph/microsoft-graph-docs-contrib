...
> [!IMPORTANT]
> This set of APIs is deprecated for most tenants except for a few that use an older version of Privileged Identity Management (PIM). For more information about PIM versions, see [Determine your version of PIM](https://docs.microsoft.com/azure/active-directory/privileged-identity-management/pim-how-to-activate-role?tabs=new#determine-your-version-of-pim). If you are using the new version and are recieving a **TenantEnabledInAadRoleMigration** error when you call the APIs, you can wait until a new set of API is available for PIM functionality under the current unifiedRoleManagement API for Azure AD roles, or you can use the current Azure Resource API for your Azure AD roles. Simply replace “azureResources” with “aadRoles” for provider_id and use your tenant id for resource id. Either of these options, we recommend that you wait for new APIs that will eventually move from beta to generally availability. Any new features made available in the UI will also be made exclusively available through the new APIs. If you must use a PIM API before the new APIs are available, you can proceed with using the current Azure Resource API for your Azure AD roles.
