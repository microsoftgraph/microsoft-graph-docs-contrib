...
|disasterRecoveryCapability|[cloudPcDisasterRecoveryCapability](../resources/cloudpcdisasterrecoverycapability.md)|The disaster recovery status of the Cloud PC, including the primary region, secondary region, and capability type. Only we select disasterRecoveryCapability property and filter disasterRecoveryCapability/{subProperty} in the request url, we can get response with disasterRecoveryCapability involved. The default value is `null` that indicates that the disaster recovery setting is disabled. Read-only. |
