...
|[Bulk reprovision remote action](../api/manageddevice-bulkreprovisioncloudpc.md) (deprecate) |None|Bulk reprovision a set of Cloud PC devices with Intune managed device IDs.|
|[Bulk reprovision](../resources/cloudpcbulkreprovision.md) |None|Bulk reprovision a set of Cloud PC devices with Cloud PC IDs.|
|[Resize remote action](../api/manageddevice-resizecloudpc.md) (deprecate)|None|Upgrade or downgrade an existing Cloud PC to another configuration with new vCPU and storage size through Intune managed device ID.This will be deprecated on Aug 31, 2024 and switch to use newly introduced resize instead.|
|[Resize](../api/cloudpc-resize.md)|None|Upgrade or downgrade an existing Cloud PC to another configuration with new vCPU and storage size through Cloud PC ID.|
...
|[Bulk restore remote action](../api/manageddevice-bulkrestorecloudpc.md) (deprecate) |[cloudPcBulkRemoteActionResult](../resources/cloudpcbulkremoteactionresult.md)|Restore multiple Cloud PC devices with a single request that includes the IDs of Intune managed devices and a restore point date and time.|
|[Bulk restore](../resources/cloudpcbulkrestore.md)|[cloudPcBulkRemoteActionResult](../resources/cloudpcbulkremoteactionresult.md)|Restore multiple Cloud PC devices with a single request that includes the CPC IDs and a restore point date and time.|
...
|[Bulk resize](../api/cloudpc-retrypartneragentinstallation.md) (deprecate) |[cloudPcRemoteActionResult](../resources//cloudpcremoteactionresult.md) collection|Perform a bulk resize action to resize a group of Cloud PCs that have successfully passed validation (cloudPC: validateBulkResize). If any devices can't be resized, they're labeled as "resize failed", while the remaining devices are `provisioned` for the resize process.|
|[Bulk resize](../resources/cloudpcbulkresize.md)|[cloudPcRemoteActionResult](../resources//cloudpcremoteactionresult.md) collection|Perform a bulk resize action to resize a group of Cloud PCs that have successfully passed validation (cloudPC: validateBulkResize). If any devices can't be resized, they're labeled as "resize failed", while the remaining devices are `provisioned` for the resize process.|
...
|[Create snapshot](../api/cloudpc-createsnapshot.md)|None|Create a [cloudPC](../resources/cloudpc.md) snapshot.|
