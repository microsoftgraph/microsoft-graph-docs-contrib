...
|frontlineCloudPcAvailability|[frontlineCloudPcAvailability](#frontlinecloudpcavailability-values)|The current availability of a frontline assigned Cloud PC. Possible values: `notApplicable`, `available`,`notAvailable` and `unknownFutureValue`. Default value is `notApplicable`. Read Only.Starting from Oct 30, 2025, this property will be deprecated and no longer be supported. Please use the new API: [retrieveFrontlineCloudPcDetail](../api/cloudpc-retrievefrontlinecloudpcdetail.md) to retrieve same details.|
