...
| Domain management operations such as associating a domain with your tenant, retrieving DNS records, verifying domain ownership, associating specific services with specific domains, and deleting domains | [domain resource type](domain.md) and its associated APIs |
...
| Manage identity providers for Microsoft Entra ID, Microsoft Entra External ID, and Azure AD B2C tenants. Also, manage federation domain configurations | [identityProviderBase resoruce type](identityproviderbase.md) and its associated APIs |
