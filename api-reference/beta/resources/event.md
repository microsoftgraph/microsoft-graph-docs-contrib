...
|cancelledOccurrences|occurrenceId Collection|For client-side expansion of recurring events, this field is returned in [Get event](../api/event-get.md) with seriesMasterId when specified in $select. Contains occurenceId of cancelled instances in a recurring series.<br>Instances in a recurring event that are cancelled are called cancelledOccurences|
...
|exceptionalOccurrences|id Collection|For client-side expansion of recurring events, this field is returned in [Get event](../api/event-get.md) with seriesMasterId when specified in $select. Contains event id and occurenceId of exceptions in a recurring series.<br>Instances that differ from their parent recurring event are called exceptions. For example, an exception may have a different subject, a different start time, or additional attendees invited only to that instance.|
...
|occurrenceId|String|This string can be constructed on client side and used to perform all operations on a specific occurrence in recurring meeting series. <br> **Format:** "OID." + seriesMasterId +"." + start date of the occurrence <br> Time zone as defined in [recurrence range](recurrencerange.md)|
