...
|languageTags|String collection|A list of locale names, each of which indicates the language of a country or region that this bookmark can be viewed in. An example of a locale is "en-US" which represents English (United States). See [supported language tags](search-api-answers-overview.md#supported-language-tags) for the current list of possible values.|
