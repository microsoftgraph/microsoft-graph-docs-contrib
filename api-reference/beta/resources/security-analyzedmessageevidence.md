...
|:---|:---|:---|
|antiSpamDirection|String|Direction of the email relative to your network. The possible values are: `Inbound`, `Outbound`, `Intra-org`.|
|attachmentsCount|Int64|Number of attachments in the email.|
|deliveryAction|String|Delivery action of the email. The possible values are: `Delivered`, `Junked`, `Blocked`, or `Replaced`.|
|deliveryLocation|String|Location where the email was delivered. The possible values are: `Inbox/Folder`, `On-premises/External`, `Junk`, `Quarantine`, `Failed`, `Dropped`, `Deleted items`.|
