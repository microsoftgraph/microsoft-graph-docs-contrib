...
|:---|:---|:---|
|action|microsoft.graph.networkaccess.filteringPolicyAction|Indicates the action taken based on filtering policies. The possible values are: `block`, `allow`, `alert`. Supports $filter (`eq`) and `$orderby`.|
