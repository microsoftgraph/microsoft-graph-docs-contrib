...
|authenticationRequirement | String | Reflects the authentication stage that was actually reached during sign-in, not the intended authentication requirement. If primary authentication fails, sign-in attempt is not evaluated by Conditional Access so the resulting value is `SingleFactorAuthentication`. <br/><br/> Supports `$filter` (`eq`, `startsWith`).|
