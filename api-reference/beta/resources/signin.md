...
|authenticationContextClassReferences|[authenticationContext](../resources/authenticationcontext.md) collection|Contains a collection of values that represent the conditional access authentication contexts applied to the sign-in. A user may satisfy the authentication context by providing a s|
...
|azureResourceId|String|Contains fully qualified Azure Resource Manager ID of an Azure resource accessed as a target of the sign in when an Azure resource was accessed.|
...
|sessionLifetimePolicies|[sessionLifetimePolicy](../resources/sessionlifetimepolicy.md) collection|Any conditional access session management policies that were applied during the sign-in event.|
