...
|authenticationProtocol|protocolType|Lists the protocol type or grant type used in the authentication. For authentications that use protocols other than the possible values listed here, the protocol type is listed as `none`. The possible values are: `none`, `oAuth2`, `ropc`, `wsFederation`, `saml20`, `deviceCode`, `unknownFutureValue`.|
...
|autonomousSystemNumber|Int32|Provides the Autonomous System Number (ASN) recorded for the actor in the sign in event. This value can be used along with IP for security detections or threat hunting.|
...
|incomingTokenType|incomingTokenType|Indicates if a token of the types listed here was presented to Azure AD to authenticate the actor in the sign in. Azure AD may have also used token types not listed in this enum to authenticate the actor; you should not infer the lack of a token if its type is not listed as a possible value. The possible values are: `none`, `primaryRefreshToken`, `saml11`, `saml20`, `unknownFutureValue`.|
...
|isTenantRestricted|Boolean|Indicates whether the signin was under a tenant restrictions policy.|
...
|uniqueTokenIdentifier|String|A unique ID used to track tokens issued by Azure AD as they are redeemed at resource providers. It is the base64 encoded request ID of a token issued by Azure AD.|
