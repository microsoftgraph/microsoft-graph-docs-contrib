...
|userExperienceType|[cloudPcUserExperienceType](../resources/cloudpcprovisioningpolicy.md#cloudpcuserexperiencetype-values)|Specifies the type of cloud object end user can access. Possible values are cloudPc and cloudApp, cloudPc indicates end user can access to the entire desktop, cloudApp indicates end user can only access apps published by Admin under this provision policy. The type will not be changeable when the provision policy has been created. It will be cloudPc by default if it's not specified when creating provisioningPolicy. Where select cloudApp as the value,  provisioningType can only be sharedByEntraGroup. Supports: $filter, $select, $orderBy. |
...
### cloudPcProvisioningType values
 ...
 The following table lists the members of an [evolvable enumeration](/graph/best-practices-concept#handling-future-members-in-evolvable-enumerations). Use the `Prefer: include-unknown-enum-members` request header to get the following values in this evolvable enum: `sharedByUser`, `sharedByEntraGroup`.
