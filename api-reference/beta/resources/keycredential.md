...
|usage|String|A string that describes the purpose for which the key can be used; for example, `None`​, `Verify`​, `PairwiseIdentifier`​, `Delegation`​, `Decrypt`​, `Encrypt`​, `HashedIdentifier`​, `SelfSignedTls`, or `Sign`. **Note:** If `usage` is `Sign`​, type should be `X509CertAndPassword`​ and `passwordCredentials​` for signing should be defined.|
