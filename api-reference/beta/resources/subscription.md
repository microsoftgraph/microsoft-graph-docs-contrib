...
| lifecycleNotificationUrl | String | Optional if `expirationDateTime` is within 1 hour; required otherwise. The URL of the endpoint that receives lifecycle notifications, including `subscriptionRemoved`, `reauthorizationRequired`, and `missed` notifications. This URL must make use of the HTTPS protocol. |
