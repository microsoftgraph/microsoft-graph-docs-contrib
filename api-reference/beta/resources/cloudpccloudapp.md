...
|:---|:---|:---|
|actionFailedErrorCode|[cloudPcCloudAppActionFailedErrorCode](#cloudpccloudappactionfailederrorcode-values)|The error code if publishing, unpublishing, or resetting a cloud app fails. Possible values are: `cloudAppQuotaExceeded`, `cloudPcLicenseNotFound`, `internalServerError`, `appDiscoveryFailed`, `unknownFutureValue`. The default value is `null`. Supports `$filter`, `$select`, `$orderBy`. Read-only.|
