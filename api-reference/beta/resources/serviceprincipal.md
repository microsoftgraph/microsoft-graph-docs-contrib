...
|isDisabled|Boolean| Specifies whether the Service Principal of an Application in a tenant or across tenants for multi-tenant apps can obtain new access tokens or access protected resources. When set to "True", existing tokens remain valid until they expire based on their configured lifetimes, and the app stays visible in the Enterprise apps list but users cannot sign in.**true** if the application is deactivated (disabled); otherwise **false**| 
