...
|capabilityStatus|String| Possible values are: `Enabled`, `Warning`, `Suspended`, `Deleted`, `LockedOut`. The **capabilityStatus** is `Enabled` if the **prepaidUnits** property has at least one unit that is **enabled**, and `LockedOut` if the customer cancelled their subscription. |
