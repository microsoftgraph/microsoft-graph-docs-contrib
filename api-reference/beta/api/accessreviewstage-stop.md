...
Stop a currently active [accessReviewStage](../resources/accessReviewStage.md). After the access review stage stops, the stage status will be in `Completed` status and the reviewers can no longer give input. If there are subsequent stages that depend on the completed stage, the next stage will be created. 
