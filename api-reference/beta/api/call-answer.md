...
Under the [Policy-based recording scenario](https://docs.microsoft.com/en-us/microsoftteams/teams-recording-policy), before a participant under policy joins a call, an incoming call notification will be sent to the bot associated with the policy.
The join information can be found under the **botData property**. The bot can then choose to answer the call and [update the recording status](https://docs.microsoft.com/en-us/graph/api/call-updaterecordingstatus?view=graph-rest-beta) accordingly.
