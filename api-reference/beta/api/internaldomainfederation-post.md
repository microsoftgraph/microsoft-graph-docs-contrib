...
|:---|:---|:---|
|displayName|String|URI that clients are redirected to when they sign out of Azure AD services. Referred to as LogOffUri in MSOnline PowerShell cmdlets. Inherited from [identityProviderBase](../resources/identityproviderbase.md). Optional.|
...
|activeSignInUri|String|URL of the end point used by active clients when authenticating with domains set up for single sign-on in Azure Active Directory (Azure AD). Single sign-on is also known as identity federation. Referred to as ActiveLogOnUri in MSOnline PowerShell cmdlets. Optional.|
