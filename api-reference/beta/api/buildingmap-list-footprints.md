...
```
> **Note:**
> * `{buildingPlaceId}` is the unique **placeId** of the [building](../resources/building.md) linked to this map in Places for which we want to retrieve the foorprint.
> * `{placeType}` must be set as `microsoft.graph.building`.
