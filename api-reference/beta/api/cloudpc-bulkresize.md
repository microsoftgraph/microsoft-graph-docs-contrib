...
Perform a bulk resize action to resize a group of [cloudPC resources](../resources/cloudpc.md) that passed [validation](cloudpc-validatebulkresize.md). If any devices can't be resized, those devices indicate "resize failed". The remaining devices are `provisioned` for the resize process.
