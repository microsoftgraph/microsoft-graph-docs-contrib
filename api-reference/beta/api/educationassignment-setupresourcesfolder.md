...
This function triggers the SharePoint resource folder creation where all file-based resources (Word, Excel, and so on) should be uploaded for a given assignment. 
Note that files must be located in this folder in order to be added as resources. Only a teacher in the class can determine what files to upload in the assignment level resource folder. 
