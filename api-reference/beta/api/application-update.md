...
| isFallbackPublicClient  | Boolean                                                                     | Specifies the fallback application type as public client. The default value is `false`, which means the fallback application type is confidential client. In scenarios where Microsoft Entra ID cannot determine the client type based on the redirect URI used, this property determines whether it is considered a public client or a confidential client. |
