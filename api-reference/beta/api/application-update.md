...
The following request shows an example updating an application's **signInAudience** from single-tenant (`AzureADMyOrg`) to multi-tenant (`AzureADMultipleOrgs`) and at the same time setting **signInAudienceRestrictions** so that the app can only be used in two allowed tenants (and the tenant where the app is registered).
