...
You can specify the following properties when creating a **learningCourseActivity** based on the course activity type.
...
|registrationId|String|ID of the provider. The ID is generated when the provider registers on viva learning using provider registration graph API’s. Required.|
...
If successful, this method returns a `201 Created` response code and a [learningCourseActivity](../resources/learningcourseactivity.md) object of any of the [learningAssignment](../resources/learningassignment.md) or [learningSelfInitiated](../resources/learningselfinitiatedcourse.md) in the response body.
