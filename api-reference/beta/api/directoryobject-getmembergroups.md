...
This API returns up to 11,000 group IDs. If more than 11,000 results are available, it returns a `400 Bad Request` error with the `Directory_ResultSizeLimitExceeded` error code. Use the [List group transitive memberOf](../api/group-list-transitivememberof.md) API instead.
