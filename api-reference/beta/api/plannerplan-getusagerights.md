...
description: "Evaluate the usage rights for a specific plan based on its sensitivity label assignment and the requesting user's permissions."
author: "ankku"
ms.date: 10/23/2025
...
Evaluate the usage rights for a specific plan based on its sensitivity label assignment and the requesting user's permissions. This method returns information about whether the plan has a sensitivity label applied and what specific actions (view, edit, print, copy, export) the user is allowed to perform on the plan content based on Rights Management Services (RMS) evaluation.
...
-->
``` http
GET https://graph.microsoft.com/beta/planner/plans/{plannerPlanId}/getUsageRights
```
