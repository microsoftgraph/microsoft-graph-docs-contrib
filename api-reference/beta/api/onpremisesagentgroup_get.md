...
<!-- { "blockType": "ignored" } -->
```http
GET /onPremisesPublishingProfiles(publishingType)/agentGroups(id1)
```
...
```http
GET https://graph.microsoft.com/v1.0/onPremisesPublishingProfiles('provisioning')/agentGroups(id1)?$expand=publishedResources,agents
```
