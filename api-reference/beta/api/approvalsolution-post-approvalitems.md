...
|allowEmailNotification|Boolean|Indicatew whether to allow email notification. Optional.|
...
|responsePrompts|String collection|Approval response prompts. The input length is two. Optional.|
...
|owner|[microsoft.approval.approvalIdentitySet](../resources/approvalidentityset.md)|The identity set of the principal who owns the approval item. Only provide the **owner** property when creating an [approvalItem](../resources/approvalitem.md) on behalf of the principal. If the owner field isn't provided, the user information from the user's context is used. Optional.|
