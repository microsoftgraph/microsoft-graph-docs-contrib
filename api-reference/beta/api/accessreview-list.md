...
If there may be many hundreds of access reviews, then improve efficiency and avoid timeouts, retrieve the result set in batches, by including both the `$top` query parameter with a page size and the `$skip=0` query parameter in a request.  When a result set spans multiple pages, Microsoft Graph returns that page with an `@odata.nextLink` property in the response that contains a URL to the next page of results.
