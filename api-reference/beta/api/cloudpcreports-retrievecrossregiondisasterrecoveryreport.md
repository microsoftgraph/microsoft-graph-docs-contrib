...
| reportName | [cloudPcDisasterRecoveryReportName](#cloudpcdisasterrecoveryreportname-values) | The report name. Use the **reportName** to get the name the Cloud PC disaster recovery report. The possible value is: `crossRegionDisasterRecoveryReport`, `disasterRecoveryReport`. When **reportName** isn't initialized, `crossRegionDisasterRecoveryReport` is the default value. |
