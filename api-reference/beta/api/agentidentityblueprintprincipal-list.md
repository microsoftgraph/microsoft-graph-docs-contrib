...
- A agent identity blueprint principal can retrieve its own agent identity blueprint and agent identity blueprint principal details without being granted any application permissions.
** TODO: Update permission and check if this block is still applicable**
- The *Application.ReadWrite.OwnedBy* permission allows an app to call `GET /applications/Microsoft.Graph.AgentIdentityBlueprint` and `GET /servicePrincipals/Microsoft.Graph.AgentIdentityBlueprintPrincipal` to list all agent identity blueprints and agent identity blueprint principals in the tenant. This scope of access has been allowed for the permission.
...
By default, this API doesn't return the value of the **key** in the **keyCredentials** property when listing all agent identity blueprint principals. To retrieve the public key info in **key**, the **keyCredentials** property must be specified in a `$select` query. For example, `$select=id,appId,keyCredentials`.

The use of `$select` to get **keyCredentials** for agent identity blueprint principals has a throttling limit of 150 requests per minute for every tenant.
...
### Example 1: Get a list of agent identity blueprint principals
