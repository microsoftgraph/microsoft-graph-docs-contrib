...
|activityType|String|Activity type. This must be declared in the [Teams app manifest](/microsoftteams/platform/overview), with the exception of `systemDefault` [reserved activity type](/graph/teams-send-activityfeednotifications), which may be used to provide free-form text in the `Actor+Reason` line of the notification.|
