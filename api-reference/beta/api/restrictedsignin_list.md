...
description: "Describes the list method of the restrictedSignIn resource (entity) from the Microsoft Graph API (v1.0 version)."
...
Provides a list of sign-ins based on tenant restrictions. Tenant Restrictions enables organizations to control access to SaaS cloud applications, based on the Azure AD tenant the applications use for single sign-on.  
...
If successful, this method returns a `200 OK` response code and collection of [restrictedSignIn](../resources/restrictedsignin.md) objects in the response body.
...
<!-- {
  "blockType": "request",
  "name": "get_restrictedsignins"
}-->
```http
GET https://graph.microsoft.com/beta/restrictedSignIns
```
...
<!-- {
  ...
  "description": "List restrictedSignIns",
  "keywords": "",
