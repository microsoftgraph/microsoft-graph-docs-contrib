...
The following tables specifiy the properties that can be updated. 
...
| customResetItNowTextText | String | String to replace the default display text of "reset it now" hyperlink inside of the sign-in form (password collection screen). This text must be Unicode and not exceed 256 characters. <br/><br/>**DO NOT USE:** Customization of "reset it now" display text is currently not supported. |
| customPrivacyAndCookiesUrl | String | String of custom URL to replace the default value of the Privacy and Cookies Url in the footer.This text must be ASCII or non-ASCII characters must be URL encoded, and not exceed 128 characters. |
