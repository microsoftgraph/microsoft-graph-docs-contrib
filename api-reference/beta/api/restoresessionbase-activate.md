...
- For the new URL restore, the restored SharePoint Online sites/OneDrive users (new URLs only) will not have the same permission as original site. Only the global admin(or SharePoint Online Admin for SharePoint Online/OneDrive & Exchange Online Admin for Exchange Online) will have permission to the restored site. However, for the in-place or same URL restore, permission will be reverted to the state of the original site at the time it's being restore form.
- Restoration of SharePoint Online sites/OneDrive users content deleted beyond recycle bin is not supported.
...
- If there are no differences between the current state of a mailbox and the prior point in time from which you're attempting a restore, a restore isn't performed.
...
- The global admin(or SharePoint Online Admin for SharePoint Online/OneDrive & Exchange Online Admin for Exchange Online) can initiate restore of up to 1000 SharePoint sites, 1000 OneDrive users, and/or 1000 mailboxes queued in parallel at a time.
