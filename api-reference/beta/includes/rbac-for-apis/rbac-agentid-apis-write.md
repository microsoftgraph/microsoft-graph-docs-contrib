...
> [!IMPORTANT]
> The AgentIdentity* permissions are currently unavailable for consent through the Microsoft Entra admin center > API permissions experience. To use these permissions, you can consent to them through Microsoft Graph API calls as described in [Grant or revoke API permissions programmatically](/graph/permissions-grant-via-msgraph?tabs=http&pivots=grant-delegated-permissions). See [Agent ID API permissions](/graph/api/resources/agentid-platform-overview) for more information about these permissions.
>
