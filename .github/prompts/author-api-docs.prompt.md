...
Before beginning the documentation process, instruct the author to add all required files to the `temp-docstubs` folder in this workspace:

**Important:** The `temp-docstubs` folder is a temporary location for easy access to files during the authoring process. After completing the documentation work, you will help the author clean up by deleting all files from this folder (except the `temp-docstubs-instructions.md` file).
...
When API changes are documented, they must be recorded in the changelog:

1. **Locate the autogenerated changelog file:**
   - Look in the `temp-docstubs` folder for the autogenerated changelog JSON file
   - These files are typically prefixed with "Microsoft." (e.g., `Microsoft.DirectoryServices.json`, `Microsoft.AAD.Reporting.json`)
   - **Important:** Do NOT use files with "Review" in their filename (e.g., `Microsoft.AAD.Reporting_Review_beta`)
   - If only review changelog files exist, ask the author to provide a valid autogenerated changelog for public-facing schema

2. **Check if changelog file exists in workspace:**
   - Search the `changelog/` folder in the workspace for a file with the same name as the autogenerated changelog stub
   - Example: If you found `Microsoft.DirectoryServices.json` in temp-docstubs, check if `changelog/Microsoft.DirectoryServices.json` exists

3. **Copy changelog records appropriately:**
   - **If the file does NOT exist in the changelog folder:** Copy the entire autogenerated changelog file from temp-docstubs to the `changelog/` folder
   - **If the file EXISTS in the changelog folder:** 
     - First, understand the JSON structure to identify the relevant record blocks
     - Copy only the new record(s) from the autogenerated changelog stub
     - Add the new record(s) to the existing changelog file in the `changelog/` folder
     - **Critical:** Do NOT overwrite any existing records in the target changelog JSON file
     - Preserve all existing changelog entries

#### How to update the What's new

When API changes are documented, they should be announced in the What's new topic:

1. **Read the What's new structure:**
   - Open `concepts/whats-new-overview.md` to understand how updates are formatted
   - Observe the existing patterns for adding new entries

2. **Determine the correct H3 heading:**
   - Use the `WorkloadArea` and `SubArea` values from the changelog you just updated
   - Apply these formatting rules:
     - If `WorkloadArea` has a value and `SubArea` is blank: Use `### {WorkloadArea}`
       - Example: WorkloadArea="Users", SubArea="" → `### Users`
     - If both `WorkloadArea` and `SubArea` have values: Use `### {WorkloadArea} | {SubArea}`
       - Example: WorkloadArea="Identity and access", SubArea="Identity and sign-in" → `### Identity and access | Identity and sign-in`

3. **Add the What's new entry:**
   - Locate or create the appropriate H3 section based on the WorkloadArea and SubArea
   - **Important:** Maintain alphabetical order of H3 titles in the file
   - Add the new entry under the appropriate H3 section
   - Follow the existing entry format in the file
   - Include a brief description of the change and link to the relevant API documentation
   - **Link formatting rules:**
     - For preview/beta changes: Append `?view=graph-rest-beta&preserve-view=true` to all links
       - Example: `/graph/api/resources/place?view=graph-rest-beta&preserve-view=true`
     - For generally available/v1.0 changes: Do NOT append any query string to links
       - Example: `/graph/api/chat-list`

#### How to update the table of contents (TOC)

For every EntityType resource page that you add with a populated Methods table, that resource must be surfaced in the appropriate `toc.mapping.json` file:

1. **Identify the correct TOC file:**
   - For beta changes: `api-reference/beta/toc/toc.mapping.json`
   - For v1.0 changes: `api-reference/v1.0/toc/toc.mapping.json`

2. **Show available top-level nodes:**
   - Read the `toc.mapping.json` file
   - Display the `name` of each available top-level node to the user
   - Ask the user which top-level node this API should be added to OR if a new top-level node should be added

3. **Handle new top-level node requests:**
   - If the user requests a new top-level node:
     - **Warn them:** "Creating a new top-level node requires approval from the Microsoft Graph docs team. In most cases, APIs should be added to existing top-level nodes."
     - Ask them to confirm their choice or choose an existing node instead

4. **Navigate existing node structure:**
   - If the user selects an existing top-level node:
     - Show them any existing `childNode` entries
     - Ask if their API should be:
       - Added directly to the current node
       - Added to an existing child node
       - Added as a new child node

5. **Add resources to the TOC:**
   - Once the target node is identified, add any EntityType resources to the `resources` array
...
3. **Execute systematically:**
   ...
   - **Phase 3 - Supporting updates:**
     ...
     - Add changelog entries (follow the changelog update process below)
     - Update What's new topic (follow the What's new update process below)
   - Track progress and mark items complete

#### How to update the changelog (for promotion scenarios)

For promotion scenarios, the autogenerated changelog stub is already in the `temp-docstubs` folder (Input 2):

1. **Use the autogenerated changelog file:**
   - The autogenerated changelog JSON file is already provided in `temp-docstubs` folder
   - These files are typically prefixed with "Microsoft." (e.g., `Microsoft.DirectoryServices.json`)
   - **Important:** Ensure it's NOT a file with "Review" in the filename (e.g., `Microsoft.AAD.Reporting_Review_beta`)

2. **Check if changelog file exists in workspace:**
   - Search the `changelog/` folder in the workspace for a file with the same name as the autogenerated changelog stub
   - Example: If the stub is `Microsoft.DirectoryServices.json`, check if `changelog/Microsoft.DirectoryServices.json` exists
...
#### How to update the What's new (for promotion scenarios)
...
3. **Add the What's new entry:**
   ...
   - Add the new entry under the appropriate H3 section
   - Follow the existing entry format in the file
   - Include a brief description noting the API was promoted from beta to v1.0
   - Link to the relevant v1.0 API documentation (not the beta documentation)
   - **Link formatting for v1.0 promotion:** Do NOT append query strings to links since promoted APIs are now generally available
     - Example: `/graph/api/chat-list` (no query string)

#### How to update the table of contents (TOC) for promotion scenarios

For every EntityType resource page being promoted that has a populated Methods table, ensure the resource is properly represented in the v1.0 TOC:

1. **Check both TOC files:**
   - The resource should already exist in `api-reference/beta/toc/toc.mapping.json`
   - You need to add it to `api-reference/v1.0/toc/toc.mapping.json`

2. **Show available top-level nodes:**
   - Read the `api-reference/v1.0/toc/toc.mapping.json` file
   - Display the `name` of each available top-level node to the user
   - Ask the user which top-level node this API should be added to
   - **Tip:** Usually, the resource should be added to the same top-level node structure as it exists in beta

3. **Handle new top-level node requests:**
   - If the user requests a new top-level node:
     - **Warn them:** "Creating a new top-level node requires approval from the Microsoft Graph docs team. In most cases, APIs should be added to existing top-level nodes that match the beta structure."
     - Ask them to confirm their choice or choose an existing node instead
...
5. **Add resources to the v1.0 TOC:**
   - Once the target node is identified, add the promoted EntityType resources to the `resources` array
   - Ensure the structure mirrors the beta TOC organization for consistency
   - If you created an API overview for v1.0, add its relative path to the `overview` property of the node
