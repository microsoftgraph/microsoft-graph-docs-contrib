...
When the author describes a small, scoped change directly in their prompt without providing formal inputs, adapt as follows:
...
**Do not block** on missing formal inputs when the author's prompt provides enough context to proceed. The author chose to skip formal inputs because the change is small — respect that by working efficiently with what they gave you.
